...
namespace IocConfig
{
    public partial class WebHelper : IWebHelper
    {
        ...
        private string _caHost;
        private string _caHostSsl;
        ...
        public virtual string GetThisPageUrl(bool includeQueryString, bool useSsl)
        {
            ...
            if (includeQueryString)
            {
                ...
                string caHost = GetCAHost(useSsl, out appPathPossiblyAppended).TrimEnd('/');
        ...
        private string GetCAHost(bool useSsl, out bool appPathPossiblyAppended)
        {
            string cached = useSsl ? _caHostSsl : _caHost;
            if (cached != null)
            {
            ...
            if (!DataSettings.DatabaseIsInstalled())
            {
                if (useSsl)
                {
                    ...
                    // So a ca owner wants it to be detected automatically.
            ...
            else
            {
                ...
                if (_currentCentralAdmin == null)
                {
                    ...
                    //if (ProjectObjectFactory.Container.GetInstance<ICAContext>()!=null) // Unit test safe!
                    //{
                    //    _currentCentralAdmin = caContext.CurrentCA;
                    //    if (_currentCentralAdmin == null)
                    //        throw new Exception("Current ca cannot be loaded");
                    //}

                    ICAContext caContext;
                    caContext = ProjectObjectFactory.Container.GetInstance<ICAContext>();
                ...
                if (_currentCentralAdmin != null)
                {
                    var securityMode = _currentCentralAdmin.GetSecurityMode(useSsl);
                    ...
                    if (httpHost.IsEmpty())
                    {
                        ...
                        result = _currentCentralAdmin.Url.EnsureEndsWith("/");
                    ...
                    if (useSsl)
                    {
                        if (securityMode == HttpSecurityMode.SharedSsl)
                        {
                            ...
                            // So a ca owner doesn't want it to be resolved automatically.
                            // In this case let's use the specified secure URL
                            result = _currentCentralAdmin.SecureUrl.EmptyNull();
                        ...
                        {
                            // Secure URL is not specified.
                            // So a ca owner wants it to be resolved automatically.
                            result = result.Replace("http:/", "https:/");
                        }
                    }
                    else // no ssl
                    {
                        if (securityMode == HttpSecurityMode.SharedSsl)
                        {
                            // SSL is enabled in this ca and shared ssl URL is specified.
                            // So a ca owner doesn't want it to be resolved automatically.
                            // In this case let's use the specified non-secure URL

                            result = _currentCentralAdmin.Url;
                            appPathPossiblyAppended = true;
                        }
                    }
                }
        ...
        public virtual string GetCALocation()
        {
            bool useSsl = IsCurrentConnectionSecured();
            return GetCALocation(useSsl);
        }

        public virtual string GetCALocation(bool useSsl)
        {
            //return HostingEnvironment.ApplicationVirtualPath;

            bool appPathPossiblyAppended;
            string result = GetCAHost(useSsl, out appPathPossiblyAppended);
