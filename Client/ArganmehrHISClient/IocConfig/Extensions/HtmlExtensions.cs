...
using DomainClasses.Entities.Catalog;
...
namespace IocConfig.Extensions
{
    ...
    public static class HtmlExtensions
    {
            ...
            where TLocalizedModelLocal : ILocalizedModelLocal
        {
            return new HelperResult(writer =>
            {
                if (helper.ViewData.Model.Locales.Count > 1)
                {
                    ...
                    var tabStrip = helper.Arganmehr().TabStrip().Name(name).SmartTabSelection(false).Style(TabsStyle.Pills).Items(x =>
                    {
                        ...
                        for (int i = 0; i < helper.ViewData.Model.Locales.Count; i++)
                        {
                            ...
                            var language = ProjectObjectFactory.Container.GetInstance<ILanguageService>().GetLanguageById(locale.LanguageId);
        ...
        public static MvcHtmlString DeleteConfirmation<T>(this HtmlHelper<T> helper, string actionName, string buttonsSelector = null) where T : EntityModelBase
        {
            ...
            var window = helper.Arganmehr().Window().Name(modalId)
                .Title(ProjectObjectFactory.Container.GetInstance<ILocalizationService>().GetResource("Admin.Common.AreYouSure"))
                .Modal(true)
                .Visible(false)
            ...
            object htmlAttributes = null)
        {
            ...
            if (resourceDisplayName != null)
            {
                ...
                if (displayHint)
                {
                    ...
                    hint = ProjectObjectFactory.Container.GetInstance<ILocalizationService>().GetResource(resourceDisplayName.ResourceKey + ".Hint", langId, false, "", true);
                }
            }
            ...
            int? selectedDay = null, int? selectedMonth = null, int? selectedYear = null, bool localizeLabels = true, bool disabled = false)
        {
            ...
            if (localizeLabels)
            {
                var locService = ProjectObjectFactory.Container.GetInstance<ILocalizationService>();
            ...
            string optionLabel = null) where TEnum : struct
        {
            ...
            var localizationService = ProjectObjectFactory.Container.GetInstance<ILocalizationService>();
            var workContext = ProjectObjectFactory.Container.GetInstance<IWorkContext>();
        ...
        //        var widgetSelector = ProjectObjectFactory.Container.GetInstance<IWidgetSelector>();
        ...
        {
            var data = helper.ViewData[ArganmehrDependingSettingHelper.ViewDataKey] as ArganmehrDependingSettingData;

            if (data != null && data.ActiveArganmehrScopeConfiguration > 0)
            {
                var settingKey = ExpressionHelper.GetExpressionText(expression);
                var localizeService = ProjectObjectFactory.Container.GetInstance<ILocalizationService>();
        ...
        public static MvcHtmlString CollapsedText(this HtmlHelper helper, string text)
        {
            ...
            var catalogSettings = ProjectObjectFactory.Container.GetInstance<CatalogSettings>();
