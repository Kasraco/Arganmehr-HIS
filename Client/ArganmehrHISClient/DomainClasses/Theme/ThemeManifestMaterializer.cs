    ...
    internal class ThemeManifestMaterializer
    {
        ...
        private Multimap<string, string> MaterializeSelects()
        {
            ...
            foreach (var xel in xndSelects)
            {
                ...
                {
                    throw new ArganmehrExeption("A 'Select' element must contain a unique id. Affected: '{0}' - element: {1}", _manifest.FullPath, xel.OuterXml);
                }

                var xndOptions = xel.SelectNodes(@"Option").Cast<XmlElement>();
                if (!xndOptions.Any())
                {
                    throw new ArganmehrExeption("A 'Select' element must contain at least one 'Option' child element. Affected: '{0}' - element: {1}", _manifest.FullPath, xel.OuterXml);
                }
                ...
                foreach (var xelOption in xndOptions)
                {
                    ...
                    if (option.IsEmpty())
                    {
                        throw new ArganmehrExeption("A select option cannot be empty. Affected: '{0}' - element: {1}", _manifest.FullPath, xel.OuterXml);
                    }
        ...
        private IDictionary<string, ThemeVariableInfo> MaterializeVariables()
        {
            ...
            foreach (var xel in xndVars)
            {
                ...
                if (info != null && info.Name.HasValue())
                {
                    ...
                    {
                        throw new ArganmehrExeption("Duplicate variable name '{0}' in '{1}'. Variable names must be unique.", info.Name, _manifest.FullPath);
                    }
        ...
        private ThemeVariableInfo MaterializeVariable(XmlElement xel)
        {
            ...
            if (name.IsEmpty())
            {
                throw new ArganmehrExeption("The name attribute is required for the 'Var' element. Affected: '{0}' - element: {1}", _manifest.FullPath, xel.OuterXml);
            }

            if (value.IsEmpty())
            {
                throw new ArganmehrExeption("A value is required for the 'Var' element. Affected: '{0}' - element: {1}", _manifest.FullPath, xel.OuterXml);
            }
        ...
        private ThemeVariableType ConvertVarType(string type, XmlElement affected, out string selectRef)
        {
            ...
            if (type.ToLower().StartsWith("select", StringComparison.CurrentCultureIgnoreCase))
            {
                ...
                {
                    throw new ArganmehrExeption("The 'id' of a select element must be provided (pattern: Select#MySelect). Affected: '{0}' - element: {1}", _manifest.FullPath, affected.OuterXml);
                }
